...
const Projects = () => {
  ...
  return (
    <section id="projects" className="py-8 sm:py-12 px-4 sm:px-6 md:px-8 rounded-none md:rounded-3xl shadow-sm md:shadow-xl bg-white dark:bg-gray-900 mx-4 md:mx-auto max-w-7xl mt-2">
      {/* Header */}
      ...
      <div className="space-y-8 sm:space-y-12 ">
        {projectData.map((project, index) => (
          <div key={index} className="flex flex-col lg:flex-row items-center gap-6 sm:gap-8 bg-gray-50 dark:bg-gray-800 rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 p-4 sm:p-6 ">
            {/* Project Image */}
            ...
            <div className="w-full lg:w-1/2 flex flex-col gap-3 sm:gap-4">
              ...
              <div className="flex flex-wrap gap-2 mt-1 sm:mt-2">
                {project.techStack.map((tool, idx) => (
                  <span key={idx} className="px-2.5 py-1 bg-primary/10 dark:bg-primary/20 text-primary dark:text-primary-300 text-xs sm:text-sm font-medium rounded-full">
                    {tool}
                  </span>
                ))}
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-4 sm:mt-6">
                <div className="flex items-center gap-4">
                  <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-gray-700 dark:text-gray-300 hover:text-primary dark:hover:text-primary-300 transition-colors duration-200 text-lg sm:text-xl">
                    <FaGithub />
                  </a>
                  <a href={project.liveSite} target="_blank" rel="noopener noreferrer" className="text-gray-700 dark:text-gray-300 hover:text-primary dark:hover:text-primary-300 transition-colors duration-200 text-lg sm:text-xl">
                    <FaExternalLinkAlt />
                  </a>
                </div>
                ...
                <button
                  ...
                  className="px-4 sm:px-5 py-2 text-sm sm:text-base rounded-full bg-primary hover:bg-primary-dark text-white transition-colors duration-200 shadow-sm hover:shadow-md cursor-pointer"
                >
                  View Details
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedProject.techStack.map((tool, idx) => (
                        <span key={idx} className="px-3 py-1 bg-primary/10 dark:bg-primary/20 text-primary dark:text-primary-300 rounded-full text-sm">
                          {tool}
                        </span>
                      ))}
                    </div>
                  ...
                  {selectedProject.roles && (
                    <div>
                      <h3 className="text-lg font-semibold mb-2">Role Features</h3>
                      <div className="space-y-3">
                        {Object.entries(selectedProject.roles).map(([role, details]) => (
                          <div key={role}>
                            <h4 className="font-medium capitalize">{role} Features:</h4>
